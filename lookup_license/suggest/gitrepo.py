from lookup_license.suggest.suggest import URLSuggester

class GitRepo(URLSuggester):
...
#            raise Exception(f'"{repo_url}" is not a repository url')
        ...
        print(f'suggest_license_files({repo_url}, {branches})')

        has_branch = self.has_branch(repo_url)

        if branches and has_branch:
            print(", ..... branch in repo url and supplied by you")
